...
import java.lang.Math;
 class GainRatio
 {
	...
	{
		System.out.println("In gain ratio - called by "+C45.currentsplit.attr_index);
		return C45.currentsplit.attr_index;
	}
	...
	//Calculation of entrophy
	  ...
	  {
		  int currentindex=0;
		  System.out.println("From gain ratio: value of currentsplit: "+C45.currentsplit.attr_index);
		  double entropy=0;
		  if(count[0][0]!=null)
		  ...
		  System.out.println("in currnodeenthopy " + classLabel);
		  ...
		  while(count[j][0]!=null && currentindex==Integer.parseInt(count[j][0]))
		  {
			  if(covered[j]==0)
		      {
				  ...
				  System.out.println("in gain ratio " + " classlabel " + classLabel+"  currnode[ind]  "+currnode[ind]);
		       }
			  else
			  {
				j++;
	...
	try {
		...
		fstream = new FileInputStream("/tmp/output/intermediate"+id.current_index+".txt");
